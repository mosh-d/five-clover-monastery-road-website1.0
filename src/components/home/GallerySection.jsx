...
export default function GallerySection() {
  ...
  const ImageWithOverlay = ({ src, alt, index, className }) => (
    <div className={`relative group cursor-pointer ${className}`} onClick={() => openModal(index)}>
      <img src={src} alt={alt} className="w-full h-full object-cover transition-all duration-300" />
      ...
      <div 
        ...
        onMouseEnter={(e) => e.currentTarget.style.backgroundColor = 'hsla(38, 50%, 10%, 0.7)'}
        onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
      >
        <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="p-[1rem]">
            <span className="text-[color:var(--white)] font-semibold text-xl">View More Images</span>
          </div>
        </div>
      </div>
    </div>
