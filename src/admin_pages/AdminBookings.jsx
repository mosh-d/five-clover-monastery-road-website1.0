...
const testLocalConnection = async () => {
  try {
    ...
    console.log(' AdminBookings: Local server not available, falling back to production');
  }
...
export default function AdminBookingsPage() {
  ...
  useEffect(() => {
    const init = async () => {
      const url = await testLocalConnection();
      setApiUrl(url);
      console.log(`AdminBookings: Using API base URL: ${url}`);
    };
    init();
  }, []);
  ...
  const confirmReservation = async (reservationId) => {
    try {
      ...
      const response = await axios.post(`${baseUrl}/api/reservations/confirm`, {
  ...
  const cancelReservation = async (reservationId) => {
    try {
      console.log('AdminBookings: Sending cancel request for reservation ID:', reservationId);
      const payload = {
        reservation_id: reservationId
      };
      ...
      const baseUrl = apiUrl.endsWith('/') ? apiUrl.slice(0, -1) : apiUrl;
      const response = await axios.post(`${baseUrl}/api/reservations/cancel`, payload, {
        headers: {
          'Content-Type': 'application/json'
        },
        withCredentials: true
      ...
      case 'hold':
        return (
          <div className="flex gap-2">
            <button 
              onClick={() => handleStatusUpdate(booking.booking_id, 'confirmed', reservationId)}
            ...
            <span>|</span>
            <button 
              onClick={() => handleStatusUpdate(booking.booking_id, 'cancelled', reservationId)}
          ...
          <table className="min-w-full border-collapse text-2xl">
            <thead>
              <tr className="border-b border-[color:var(--text-color)]">
                <th className="px-8 py-4 text-left whitespace-nowrap">Guest Name</th>
                <th className="px-8 py-4 text-left whitespace-nowrap">Booking ID</th>
                <th className="px-8 py-4 text-left whitespace-nowrap">Check In Date</th>
                <th className="px-8 py-4 text-left whitespace-nowrap">Check Out Date</th>
                <th className="px-8 py-4 text-left whitespace-nowrap">Room Category</th>
                <th className="px-8 py-4 text-left whitespace-nowrap">No of Rooms</th>
                <th className="px-8 py-4 text-left whitespace-nowrap">Status</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan="8" className="px-8 py-8 text-center text-red-600">
                    {error}
                  </td>
                </tr>
              ) : bookings.length === 0 ? (
                ...
                bookings.map((booking) => (
                  <tr key={booking.booking_id} className="border-b border-[color:var(--text-color)]">
                    <td className="px-8 py-4 text-left">{booking.guest_name}</td>
                    <td className="px-8 py-4 text-left">{booking.booking_id}</td>
                    <td className="px-8 py-4 text-left">{formatDate(booking.check_in_date)}</td>
                    <td className="px-8 py-4 text-left">{formatDate(booking.check_out_date)}</td>
                    <td className="px-8 py-4 text-left">{booking.room_category}</td>
                    <td className="px-8 py-4 text-left">{booking.no_of_rooms}</td>
                    <td className="px-8 py-4 text-left capitalize">{booking.status.toLowerCase()}</td>
